...
void update_z_fam(double *beta, double *freq, double D, int per_y_new, int *per_xzc, int *per_xzu, int **per_xchaplo, int **per_xuhaplo, int num_haplo_per, int x_length);

void update_freq_fam(double *freq, double *beta, double D, int x_length, int N, int n_cases, int tot_uniq_x, int **xz, int n_iter);

void update_beta_fam(int **xz, double *beta, int which_beta, double lambda, double *freq, double D, int x_length, int *y_new, int n_cases, int tot_uniq_x, int N);

double update_D_fam(double *freq, double *beta, double D, int x_length, int N, int n_cases, int tot_uniq_x, int **xz, int n_iter);

int **uniq_x_mat, *y_new;
//total_freq_update=0;

void famLBLmcmc(int *x, int *tot_hap, int *y, int *N, int *num_haplo_id, int *x_length, double *freq, double *D, double *beta, double *a, double *b, int *uniq_x, 
  ...
  for (n=0; n<*NUM_IT; ++n) {
    ...
    for (i =0; i<n_cases; ++i) {
      if (num_haplo_id[i]>1) {
		    update_z_fam(beta, freq, *D, y_new[i], xz[i], xz[i+n_cases], xhaplo[i], xhaplo[i+n_cases], num_haplo_id[i], *x_length);
	    }
    }
    ...
    for (i=0; i<*x_length; ++i) {
      which_beta=i;
      update_beta_fam(xz, beta, which_beta, *lambda, freq, *D, *x_length, y_new, n_cases, *tot_uniq_x, *N);
    }   
    ...
    *lambda = update_lambda(beta, *a, *b, *x_length);
    ...
    update_freq_fam(freq, beta, *D, *x_length, *N, n_cases, *tot_uniq_x, xz, n);
      
     /* update D parameter */
      
    *D = update_D_fam(freq, beta, *D, *x_length, *N, n_cases, *tot_uniq_x, xz, n);
...
void update_freq_fam(double *freq, double *beta, double D, int x_length, int N, int n_cases, int tot_uniq_x, int **xz, int n_iter) {
  ...
  if (-min_f_new/(1-min_f_new)  < D) {
    ...
    for (i=0; i<n_cases; ++i) {
      g_old += log(calc_a(freq, xz[i], x_length, D)) + log(calc_a(freq, xz[n_cases + i], x_length, D));
      g_new += log(calc_a(prop_freq_with_last, xz[i], x_length, D)) + log(calc_a(prop_freq_with_last, xz[n_cases+ i], x_length, D));
    }
...
double update_D_fam(double *freq, double *beta, double D, int x_length, int N, int n_cases, int tot_uniq_x, int **xz, int n_iter) {
  ...
  for (i=0; i<n_cases; ++i) {
    g_old += log(calc_a(freq, xz[i], x_length, D)) + log(calc_a(freq, xz[i+n_cases], x_length, D));
    g_new += log(calc_a(freq, xz[i], x_length, prop_D)) + log(calc_a(freq, xz[i+n_cases], x_length, prop_D));
  }
